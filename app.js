...
var connector = new builder.ChatConnector({
    ...
    //appId: "d8097255-6623-4e14-bf66-54d504d05319",
    //appPassword: "yfkL74-{gcriZDZTKC601*["
});
...
var bot = new builder.UniversalBot(connector, [
    ...
    function (session, results) {
        session.dialogData.srccur = results.response.toString().split(" ")[1];
        amount = results.response.toString().split(" ")[0];
...
bot.dialog('askForBaseCUR', [
    function (session) {
        builder.Prompts.text(session, "What is your target Currency code?");
    },
    function (session, results) {
        session.endDialogWithResult(results);
    }
...
bot.dialog('askForTargetCUR', [
    function (session) {
        builder.Prompts.text(session, "How much you wanted to invest in this?(Ex., 250 USD or 320 CAD)");
    },
    function (session, results) {
        session.endDialogWithResult(results);
    }
    ...
    function (session) {
        request(url, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                console.log("Traded amount:"+convertionrate*amount);
                //session.send("The current Conversion rate for trading is :"+obj[cur_code]['val']);
                
                console.log("ending");
                builder.Prompts.choice(session, "The Current Conversion rate is :"+convertionrate*amount+", what's your take on this?", "confirm|reject", { listStyle: builder.ListStyle.button });
            }
        });
